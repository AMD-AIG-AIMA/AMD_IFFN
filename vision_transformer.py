...
class LearnableScaleBias(nn.Module):
    def __init__(self, out_chn):
        super(LearnableScaleBias, self).__init__()
        self.scale = nn.Parameter(torch.ones(1,1,out_chn), requires_grad=True)
        self.bias = nn.Parameter(torch.zeros(1,1,out_chn), requires_grad=True)
        
    def forward(self, x):
        out = x * self.scale.expand_as(x) + self.bias.expand_as(x)
        return out

class MyACT(nn.Module):
    def __init__(self, out_chn):
        super(MyACT, self).__init__()
        self.move1 = LearnableScaleBias(out_chn)
        self.act = nn.GELU()
        self.move2 = LearnableScaleBias(out_chn)
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., args=None):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.args = args
        self.act1 = MyACT(hidden_features)
        self.act2 = MyACT(hidden_features)
        self.drop = nn.Dropout(drop)
        self.conv1 = nn.Linear(in_features, hidden_features)
        self.conv2 = nn.Linear(hidden_features*2, out_features)
        self.ln1 = nn.LayerNorm(hidden_features * 2)
        self.ln2 = nn.LayerNorm(out_features)

        self.act = nn.GELU()
        self.ln = nn.BatchNorm2d(hidden_features * 2)
        if 'base' in args.model:
            self.conv5x5_dw = nn.Conv2d(hidden_features * 2, hidden_features * 2, kernel_size=5, padding=2, groups=hidden_features * 2)
        else:
            self.conv3x3_dw = nn.Conv2d(hidden_features * 2, hidden_features * 2, kernel_size=3, padding=1, groups=hidden_features * 2)

    def forward(self, x):
        x = self.conv1(x)
        x = torch.cat((self.act1(x), self.act2(x)), dim=2)
        x = self.ln1(x)
        x = self.drop(x)
        ...
        if 'base' in self.args.model:
            x = x + self.act(self.ln(self.conv5x5_dw(x)))
        else:
            x = x + self.act(self.ln(self.conv3x3_dw(x)))
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, args=args)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, args=None, pretrained_cfg=None):
        super().__init__()
